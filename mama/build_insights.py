...
def _start_reason(output: str) -> str:
    ...
    if 'preventing vcperf' in low or 'failed to start' in low:
        return 'another ETW trace is already running (try `vcperf /stop mama_buildstats` or elevated `xperf -stop`)'
...
class VcPerfSession:
    ...
    def __enter__(self):
        ...
        if st != 0:  # otherwise maybe a stale session from a crashed run - clear it and retry once
            ...
            st, out = self._start()
        self.ok = st == 0
        if not self.ok: warning(f'buildstats: {_start_reason(out)}; skipping Build Insights')
        return self
    ...
    def __exit__(self, *exc):
        if self.ok:
            ...
            if not self.ok: warning('buildstats: vcperf produced no trace, skipping Build Insights')
...
def print_buildstats_deep(stats: TraceStats, scope_label: str):
    ...
    if stats.empty:
        console('    no compiler activity captured (nothing recompiled - try `rebuild buildstats`)')
        return
