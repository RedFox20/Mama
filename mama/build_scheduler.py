...
def build_dep_jobs(deps, configure_fn, build_fn, weight_fn=None, children_fn=None) -> List[Job]:
    ...
    bld = {d: Job((d, 'b'), BUILD, (lambda x=d: build_fn(x)), weight=(lambda x=d: weight_fn(x)),
                  node=d, ungated=d.is_root) for d in deps}
    for d in deps:
...
class Scheduler:
    def __init__(self, *, max_configure: int, core_budget: int, max_load: int = 20,
        ...
        self._gen = 0            # bumped on every state change, so a pass can tell it raced the display
        ...
        with self._cond:
            ...
            while self._pending or self._n_running:
                ...
                if self._pending_log is not None:
                    # Terminal I/O must NOT hold the scheduler lock: a completing job takes the same lock,
                    # so a frame write would stall every launch and completion behind it.
                    hint, gen = self._pending_hint(blocked), self._gen
                    self._cond.release()
    ...
    @staticmethod
    def _name(job: Job):
        return job.node.name if job.node else str(job.key)  # node is optional on a bare Job
