...
from .buildsys.cmake.scan import MAMA_CMAKE, proxy_paths
...
class BuildDependency:
    ...
    def mama_cmake_paths(self) -> list:
        """Every path a proxy `include()` names. The scan never caches: a configure() hook can rewrite
        the CMakeLists.txt with no change a stat sees. `buildsys/cmake/scan.py` holds the contract."""
        cmake_dir = self.cmake_source_dir()
        return proxy_paths(self.cmakelists_path(), cmake_dir, self.src_dir, self.name)
