...
process - the 5.7s/sample regression), and the Windows kernel32 path's correctness + speed."""
...
def test_psutil_sampler_sums_tree_and_touches_only_that_tree(monkeypatch):
    ...
    # read cpu only for the build's own 2 procs, never enumerate the whole system (the 5.7s/sample bug).
