...
def test_toolchain_inputs_cover_the_cross_setup_but_never_project_flags(tmp_path, monkeypatch):
    ...
    assert out['CMAKE_AR'] == '/opt/sdk/bin/aarch64-ar'        # cross binutils shape detection too
    assert out['CMAKE_TOOLCHAIN_FILE']['size'] == tc.stat().st_size  # stat'd: an edit in place invalidates
...
def test_cmake_defines_survive_a_windows_path(tmp_path):
    ...
    assert defines.strip() == '-DPROTOC_EXECUTABLE=C:/proj/packages/protobuf/windows/bin/protoc.exe'
    assert shlex.split(f'cmake {defines}')[1].endswith('/bin/protoc.exe')  # survives the split intact


def test_cmake_defines_leave_msvc_flags_alone():
    assert cc._opts_to_defines(['CMAKE_CXX_FLAGS=/EHsc /MP']).strip() == '-DCMAKE_CXX_FLAGS=/EHsc /MP'


def test_cmake_defines_only_rewrite_path_shaped_backslashes():
    # a literal separator or an escaped quote is not a path: rewriting those changes what the compiler sees
    assert cc._opts_to_defines([r'SEP=\\']).strip() == r'-DSEP=\\'
    assert cc._opts_to_defines([r'NAME=\"q\"']).strip() == r'-DNAME=\"q\"'
    assert cc._opts_to_defines([r'P=C:\a\b.exe']).strip() == '-DP=C:/a/b.exe' 
