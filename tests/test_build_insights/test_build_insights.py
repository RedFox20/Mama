...
def test_deep_report_demangles_codegen_symbols(capsys):
    ...
    tr = {'traceEvents': [
        ...
        _e(6_000_000)]}
    bi.print_buildstats_deep(bi.parse_timetrace(tr), 'root')
    out = strip_ansi(capsys.readouterr().out)
...
def test_session_issues_start_noadmin_nocpusampling_and_stop_timetrace(monkeypatch, tmp_path):
    ...
    assert any('/start /noadmin /nocpusampling /level3 mama_buildstats' in c for c in flat)
    assert any(f'/stop mama_buildstats /timetrace {out}' in c for c in flat)
