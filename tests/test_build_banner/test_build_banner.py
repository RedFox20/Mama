...
def test_toolchain_names_the_clang_stdlib_on_linux(capsys):
    ...
    assert _banner(capsys, _cfg(msvc=True, linux=False, name=lambda: 'windows')) \
...
def _android(**over):
    ...
    a = SimpleNamespace(android_api='android-36')
    a.android_ndk = lambda: '/Android/ndk/29.0.14206865'
    return _cfg('/Android/ndk/29.0.14206865/bin/aarch64-linux-android36-clang', '21.0.0',
                linux=False, android=a, arch='arm64', **over)
...
def test_platform_is_dropped_rather_than_failing_the_banner(capsys):
    ...
    broken = _android()
    broken.android.android_ndk = lambda: (_ for _ in ()).throw(RuntimeError('no ndk'))
