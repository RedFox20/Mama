...
def test_an_empty_proxy_is_a_truncated_write_and_mama_replaces_it(tmp_path):
    # a zero-byte mama.cmake holds nothing a user typed, and keeping it would strand every MAMA_ variable
    dep = _includes_proxy(_dep(tmp_path, 'leaf'))
    write_files(dep.src_dir, {'mama.cmake': ''})
    dc._save_cmake_files(dep)
    assert 'auto-generated by mama' in open(f'{dep.src_dir}/mama.cmake').read()
