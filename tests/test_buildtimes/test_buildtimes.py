...
def test_bar_normalizes_to_slowest_and_pads_to_full_width():
    full = strip_ansi(dc._buildtimes_bar({'build': 100.0}, 100.0, 100.0, dc._GLYPHS_ASCII))
    half = strip_ansi(dc._buildtimes_bar({'build': 50.0}, 50.0, 100.0, dc._GLYPHS_ASCII))
...
def test_bar_segments_are_proportional_in_load_cfg_build_order():
    bar = strip_ansi(dc._buildtimes_bar({'load': 1.0, 'configure': 1.0, 'build': 2.0}, 4.0, 4.0, dc._GLYPHS_ASCII))
