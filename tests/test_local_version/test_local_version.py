...
@pytest.mark.parametrize('rel_path', ['mama.cmake', 'cmake/mama.cmake'])
def test_the_generated_mama_cmake_never_changes_the_version(tmp_path, rel_path):
    """The build writes it between the download hash and the upload hash, so the two must agree."""
    dep = _dep(tmp_path)
    before = _version(dep)
    generated = Path(dep.src_dir) / rel_path
    generated.parent.mkdir(parents=True, exist_ok=True)
    generated.write_text('# generated by mama\n')
    assert _version(dep) == before
