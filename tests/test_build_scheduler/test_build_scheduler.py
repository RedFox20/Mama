...
class _Dep:
    def __init__(self, name, children=()):
        self.name = name; self._children = list(children); self.is_root = False
...
def test_pending_log_is_called_without_holding_the_scheduler_lock():
    ...
    def probe(hint):
        ...
        held.append(not got[0])   # True = the scheduler still held the lock
    sched = _sched(pending_log=probe)
    done = threading.Event()
    jobs = [Job('a', BUILD, lambda: done.wait(0.3), node=SimpleNamespace(name='a')),
