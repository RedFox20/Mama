...
A file that does not start with `# This file is auto-generated by mama build.` keeps its contents,
and the run prints one warning. So a scan that names one path too many costs a warning, never a
hand-written file.
