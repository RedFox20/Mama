...
A file whose first line does not start with `# This file is auto-generated by mama build.` keeps its
contents, and the run prints one warning. Mama writes over an EMPTY file, because a zero-byte proxy
...
such as an `include()` inside a bracket comment, writes one generated file that nothing reads. The
write rule above stops it from replacing a hand-written one.
